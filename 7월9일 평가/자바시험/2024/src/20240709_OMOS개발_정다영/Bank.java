...
public class Bank {
    ...
    public String clientInfo(int clientNumber, String name){
        for (int i =0; i<clientList.size(); i++){
            if(clientList.get(i).getClientNumber()==clientNumber&&clientList.get(i).getName().equals(name)){
                return clientList.get(i).toString();
            }
        }
        return ("null");
    ...
    //메서드를 정의하시오.
    public List<String> exceedBal(int balance){
        List<String> client = new ArrayList<>();
        for (int i = 0; i<clientList.size(); i++){
            if (clientList.get(i).getAccInfo().getBalance()>balance){
                client.add(clientList.get(i).getAccInfo().toString());
            }
        }
        return client;
