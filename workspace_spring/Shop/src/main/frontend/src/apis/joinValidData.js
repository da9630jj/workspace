...
export const joinValidate = (newData, valid_tag, tagName) => {
  ...
  //id 값을 변경했으면 id피드백만 띄움
  //pw 값을 변경했으면 pw피드백만 띄움
  //name 값을 변경했으면 name피드백만 띄움
  //tel 값을 변경했으면 tel피드백만 띄움
  switch(tagName){
    case 'memID':
    ...
    case 'memPW':
      case 'checkPW':
        ...
        if(regex_memPW.test(newData.memPW)){
          ...
          sendFeedbackMsg(valid_tag[1], '비밀번호 형식을 확인하세요.', 'error');
          result_pw1=false;
        }

        //입력한 값이 다르면
        if(newData.memPW != newData.checkPW){
